CI_DOPi<-function(data){
  ...
  CI_Porg_model<-function(time, state, pars){
    ...
    with(as.list(c(state, pars)),{
      ...
      #equations
      Ef=Vmax*S/(Kmf*(1+SRP/Kic) + S)*(Ki/(Porg+Ki))#fluorescent product
      Es=Vmax*Porg/(Kmorg*(1+SRP/Kic) + Porg)
    ...
    for(i in unique(data$Substrate)){
      for(n in unique(data$InhibitorSRP)){
        out<-as.data.frame(ode(y=c(Pf=0, S=i, SRP=n,
                               ...
                               parms = c(Vmax=x[1], Kmf=x[2], Kic=x[3], Kmorg=x[4], Ki=x[5]), 
  ...
  par_mcmc<-modMCMC(f=cost, p=c(1e-2, 20, 20, 20, 20), 
                    lower=c(1e-3, 1e-2, 1e-2, 1e-2, 1e-2),
                    upper=c(100, 500, 500, 500, 500), niter=10000)
  ...
  names(Parameters)<-c("Vmax", "Kmf", "Kic", "Kmorg", "Ki")
