hill<-function(data){
  ...
  #Define the model
  hillinhibition<-function(time, state, pars){
    
    with(as.list(c(state, pars)),{
      
      #Fluorescent product/substrate
      dPf<-Vmax*S/(Km + S)*((Pt/Kic)^n/(1 + (Pt/Kic)^n))#fluorescent product
      dS<--Vmax*S/(Km + S)*((Pt/Kic)^n/(1 + (Pt/Kic)^n))
      dPt<-Vmax*S/(Km + S)*((Pt/Kic)^n/(1 + (Pt/Kic)^n))#P-PO4
    ...
    for(i in unique(data$Substrate)){
      for(n in unique(data$InhibitorSRP)){
        out<-as.data.frame(ode(y=c(Pf=0, S=i, Pt=n), 
                               ...
                               hillinhibition, times=sort(unique((data[(data$Substrate==i & data$InhibitorSRP==n), "time"]))))) 
