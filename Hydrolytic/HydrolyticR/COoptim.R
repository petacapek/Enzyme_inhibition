COoptim <- function(data){
  ...
  Cost <- function(x){
    Yhat <- matrix(unlist(lapply(1:dim(Uqs)[1], function(i){
      scpy$odeint(CO, y0 = c(Uqs[i,1], Uqs[i,2], 0), t = as.numeric(RT[,i]), args = tuple(x))[,3]})), ncol = dim(Uqs)[1])
    return(sum((Obs - Yhat)^2/2/W^2, na.rm = T))
  }
  
  ##First guess of model parameters by MCMC 
  Guess <- ifelse(data$Enzyme == "Beta-Glucosidase",
                  modMCMC(Cost, p = c(1, 10, 1), lower = c(1e-3, 1e-1, 0.5), upper = c(10, 100, 1.5), niter = 10000),
                  modMCMC(Cost, p = c(0.01, 10, 1), lower = c(1e-5, 1e-1, 0.5), upper = c(1, 100, 1.5), niter = 10000))
  ##Estimate
