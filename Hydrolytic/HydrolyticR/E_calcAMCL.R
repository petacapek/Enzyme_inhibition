E_calcAMCL<-function(dataset, MUBconc, APconc, Iconc, Nmeasure, DW, empty){#
  ...
  # }
  #==========================================
  #For AMCL Substrate, polynomial regression is used instead
  for(i in unique(contr$Date)){
   CalPolyContr <- lm(C_MUB ~ poly(E, 2, raw = T), data = calsCntr[calsCntr$Date == i, ])
   contr[contr$Date==i, "Pcontr"] <- as.numeric(predict(CalPolyContr, newdata = contr[contr$Date==i, ]))
  }
  ...
  for(i in unique(d$Date)){
    CalPoly <- lm(C_MUB ~ poly(E, 2, raw = T), data = calsSl[calsSl$Date == i, ])
    d[d$Date==i, "P"] <- as.numeric(predict(CalPoly, newdata = d[d$Date==i, ]))
  }
  ...
  d$InhibitorInitial <- ifelse(d$InhibitorInitial<0, 0, d$InhibitorInitial)
